...
export function eraser(strokes: Stroke[], center: Point, rad: number) {
  return strokes.filter((stroke) => stroke.points.every((point) => {
    if (Math.sqrt((point.x - center.x) ** 2 + (point.y - center.y) ** 2) < rad) {
      return false;
    }
  }))
}
